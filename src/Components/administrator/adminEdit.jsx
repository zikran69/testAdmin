...
export default function AdminEdit({ idData }) {
  const [showModal3, setShowModal3] = useState(false);
  const myId = idData;
  useEffect(() => {
    fetch("http://localhost:8000/admin/" + myId)
      .then((res) => {
        return res.json();
      })
      .then((resp) => {
        ...
        setNik(resp.nik);
        setName(resp.name);
        setEmail(resp.username);
        setPassword(resp.password);
        setAddress(resp.address);
        setPhone(resp.phone);
        setLast(resp.lastLogin);
  ...
  return (
    <>
      <button
        ...
        title="edit"
        className="form-edit-data py-1 px-5 bg-yellow-400 rounded-md hover:bg-hover-yellow"
      >
            ...
            <div className="relative w-auto my-6 mx-auto max-w-3xl">
              ...
              <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                ...
                <div className="flex items-start justify-between p-5 border-b border-solid border-slate-200 rounded-t">
                  <h3 className="text-3xl font-semibold">Form Edit Admin</h3>
                  <button
